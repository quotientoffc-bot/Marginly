...
(Math as any).map = function (n: number, start: number, stop: number, start2: number, stop2: number) {
...
class AsciiFilter {
  ...
  constructor(renderer: any, { fontSize, fontFamily, charset, invert }: any = {}) {
  ...
  asciify(ctx: any, w: number, h: number) {
    if (ctx && w && h) {
...
class CanvasTxt {
  ...
  constructor(txt: string, { fontSize = 200, fontFamily = 'Arial', color = '#fdf9f3' }: any = {}) {
    this.canvas = document.createElement('canvas');
    this.context = this.canvas.getContext('2d');
    this.txt = txt;
...
export default function ASCIIText({
  ...
  useEffect(() => {
    ...
    const createAndInit = async (container: any, w: number, h: number) => {
